...
class Allergens_Dietary_Pro_Filter
{
	...
	public static function instance()
	{
		if (is_null(self::$_instance)) {
			self::$_instance = new Allergens_Dietary_Pro_Filter();
		}
	...
	private function __construct()
	{
		...
		$this->_allergens = Allergens_Dietary_Pro_Allergen_Queries::getInstance()->getAllAllergens();
	}
	...
	public function create_filter()
	{
		...
		$html .= '<button class="filter-button" id="ictoria-filter-dropdown-button">' . __('Show allergen filters', 'allergens-dietary-pro') . '</button>';
		...
		$html .= '<h3>' . __('Allergens', 'allergens-dietary-pro') . '</h3>';
		$html .= '</div>';
		...
		foreach ($allergen_arr as $allergen) {
			...
			$html .= '<label for="' . $allergen['allergy_name'] . '" >' . __('No ', 'allergens-dietary-pro') . $allergen['allergy_name'] . '</label>';
			$html .= '</div>';
		}
		$html .= '</div>';
		$html .= '</div>';
		$html .= '<div class="checkbox-container">';
		$html .= '<div class="filter-header">';
		$html .= '<h3>' . __('Dietary restrictions', 'allergens-dietary-pro') . '</h3>';
		$html .= '</div>';
		$html .= '<div class="checkbox-group">';
		foreach ($diet_arr as $diet) {
			...
			$html .= '<label for="' . $diet['allergy_name'] . '" >' . __($diet['allergy_name'], 'allergens-dietary-pro') . '</label>';
			$html .= '</div>';
		}
		$html .= '</div>';
		$html .= '</div>';
		...
		$html .= '<button type="submit" name="allergen_filter" class="filter-button">' . __('Apply Filters', 'allergens-dietary-pro') . '</button>';
		$html .= '<a href="' . get_permalink(wc_get_page_id('shop')) . '"  class="filter-button filter-reset" onclick="return confirmResetInput();">' . __('Clear Filters', 'allergens-dietary-pro') . '</a>';
		$html .= '</div>';
		$html .= '</form>';
		$html .= '</div>';
	...
	public function filter_query($query)
	{
		if ($query->is_main_query() && is_shop() && isset($_POST['allergen_filter'])) {
			...
			if (! empty($selected_options)) {
				$filtered_products = Allergens_Dietary_Pro_Allergy_Product_Queries::getInstance()->getFilteredProducts($selected_allergens, $selected_diatary);
				$product_arr = array();
