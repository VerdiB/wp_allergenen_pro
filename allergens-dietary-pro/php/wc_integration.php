...
class Allergens_Dietary_Pro_Wc_Integration_Settings extends WC_Integration {

	public function __construct() {
		global $woocommerce;
		$this->id                 = 'allergens_dietary_ictoria';
		$this->method_title       = __( 'Allergens and Dietary Ictoria plugin settings', 'allergens-dietary-pro' );
		...
		add_filter( 'plugin_action_links_' . ALLERGENS_DIETARY_PRO_BASE, array( $this, 'plugin_action_links' ) );
	}
	...
	public function generate_allergensdietary_html() {
		$options = Allergens_Dietary_Pro_Functions::get_options();
		...
		<p class="settings_header">' . __( 'Enable and disable allergens and dietary options on the whole website. This only changes the availability of the options and does not add or remove them from products', 'allergens-dietary-pro' ) . ':</p>';
		...
		foreach ( $categories as $key => $value ) {
			...
			$html .= '<div class="allergen-div">
				<p>
					...
					' . __( ucfirst( $key ), 'allergens-dietary-pro' ) . ':
				</p>
				' . $value . '
			</div>';
		}
	...
	public static function plugin_action_links( $links ) {
		$settings_link = sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=wc-settings&tab=integration&section=allergens_dietary_ictoria' ), __( 'Settings', 'allergens-dietary-pro' ) );
