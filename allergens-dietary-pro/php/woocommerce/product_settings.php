...
class Allergens_Dietary_Pro_Product_Settings
{
	...
	public static function instance()
	{
		if (is_null(self::$_instance)) {
			self::$_instance = new Allergens_Dietary_Pro_Product_Settings();
		}
	}
	...
	public function data_tab($product_data_tabs)
	{
		$product_data_tabs['allergens-tab'] = array(
			'label' => __('Allergens', 'allergens-dietary-pro'),
	...
	{
		global $post;
		$options = Allergens_Dietary_Pro_Allergy_Attachment_Queries::getInstance();
		$allergens = $options->getAllAllergyAttachmments();
		$this->_attachedAllergens = Allergens_Dietary_Pro_Allergy_Product_Queries::getInstance()->getAllergyProduct($post->ID);
		...
		$html = '<div id="allergens_dietary_ictoria_product_data" class="panel woocommerce_options_panel">
			<h2>' . __('Select allergen(\'s) and/or dietary restrictions:', 'allergens-dietary-pro') . '</h2>';
	...
	public function save_product_options($post_id)
	{
		...
		$this->_attachedAllergens = Allergens_Dietary_Pro_Allergy_Product_Queries::getInstance()->getAllergyProduct($post_id);
		...
		if ($old_diff) {
			foreach ($old_diff as $allergen) {
				Allergens_Dietary_Pro_Allergy_Product_Queries::getInstance()->deleteAllergyProduct($post_id, $allergen);
			}
		}
		// If the old doesn't contain allergens from the new one, Add the new.
		if ($new_diff) {
			foreach ($new_diff as $allergen) {
				Allergens_Dietary_Pro_Allergy_Product_Queries::getInstance()->addAllergyProduct($post_id, $allergen);
			}
		}
