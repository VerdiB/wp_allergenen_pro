...
class Allergens_Dietary_Ictoria_Allergen_Form implements I_Allergens_Dietary_Ictoria_Form
{
	...
	public function showForm(?string $allergenName = null)
	{
		...
		if (in_array($page, $showOnPage, true)) {
			...
			$html .= '<textarea class="update_" name="allergen_description" id="allergen_description" style="width: 300px; height: 70px" disabled>'. ((!empty($this->_allergen)) ? $this->_allergen['allergy_description'] : '') . '</textarea><br><br>';
			$html .= '</div></fieldset>';
			...
			$html .= '</div><input disabled type="submit" class="update_ button button-primary save" name="submit" class="button button-primary" value="' . __('Update', 'allergens-dietary-ictoria') . '"/></fieldset">';
		} else {
			$html = '<fieldset>';
